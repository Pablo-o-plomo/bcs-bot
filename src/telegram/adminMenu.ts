...
export function getMenuKeyboard(command: string): TelegramBot.SendMessageOptions['reply_markup'] {
  ...
  if (command === '/ai_signal') return {
    inline_keyboard: [
      [{ text: '✅ Рассмотреть вход', callback_data: 'signal_enter' }, { text: '❌ Пропустить', callback_data: 'signal_skip' }],
  ...
  if (command === '/signal_enter') return { inline_keyboard: [[{ text: '✅ Подтвердить сделку', callback_data: 'signal_confirm' }, { text: '❌ Отмена', callback_data: 'signal_cancel' }], [{ text: '🏠 Главное меню', callback_data: 'menu_home' }]] };
