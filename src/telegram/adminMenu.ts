...
export function getMenuKeyboard(command: string): TelegramBot.SendMessageOptions['reply_markup'] {
  ...
  if (command === '/signal_enter') {
    return {
      inline_keyboard: [
        [{ text: '✅ Подтвердить', callback_data: 'signal_confirm' }, { text: '❌ Отмена', callback_data: 'signal_cancel' }],
        [{ text: '🏠 Главное меню', callback_data: 'menu_home' }],
      ],
    };
  }

  if (['/signal_skip', '/signal_confirm', '/signal_cancel'].includes(command)) {
    return withHome([[{ text: '🧠 Новый сигнал', callback_data: 'ai_signal' }]]);
  }

  if (command === '/settings' || command === '/risk') {
    return {
      inline_keyboard: [
  ...
  if (['/risk_status', '/paper', '/paper_mode', '/emergency_stop'].includes(command)) return getBackHomeKeyboard('risk');
  if (['/journal', '/diary', '/daily_report', '/monthly_report', '/commissions', '/export', '/watchlist', '/help'].includes(command)) return getBackHomeKeyboard('risk');
