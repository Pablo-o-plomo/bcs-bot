...
export function validateBcsTokenConfig(): void {
  if (!config.bcsApi.enabled) {
    throw new Error('BCS API disabled');
  }

  if (!config.bcsApi.token) {
    throw new BcsAuthError('BCS API token is not configured');
  }

  if (!config.bcsApi.accountId) {
    logger.warn('BCS_ACCOUNT_ID is not configured; portfolio sync may still work but account verification is incomplete');
  }

  if (config.bcsApi.clientId !== 'trade-api-read') {
    logger.warn('BCS_CLIENT_ID is not trade-api-read; READ_ONLY_MODE guard is enforced');
  }
}
...
export async function getBcsAccessToken(forceRefresh = false): Promise<string> {
  ...
  if (!forceRefresh && cache && cache.expiresAt - 60_000 > now) {
    return cache.accessToken;
  }

  const authUrl =
    config.bcsApi.authUrl ||
    `${config.bcsApi.baseUrl}/trade-api-keycloak/realms/tradeapi/protocol/openid-connect/token`;
  ...
  try {
    ...
    if (!accessToken) {
      throw new BcsAuthError('BCS auth response does not include access_token');
    }
    ...
    logger.info(`BCS API auth: access token refreshed, expiresIn=${expiresIn}s`);
    ...
    logger.error(`BCS API auth error: ${sanitizeSecret(message)}`);
...
export function sanitizeBcsError(err: any): string {
  ...
  return [
    ...
    type ? `type=${sanitizeSecret(type)}` : undefined,
