...
2. Добавьте в окружение:
   ...
   - `READ_ONLY_MODE=true`
   - `ALLOW_ORDER_EXECUTION=false`
3. Бот никогда не выводит и не логирует токен. Метод отправки заявки в API-клиенте всегда выбрасывает `Order execution disabled`.
4. Если API недоступен, бот не падает: портфель показывает последний sync или локальный дневник.
...
- при попытке реального исполнения без разрешения будет ошибка `Order execution disabled`.

Telegram-меню показывает `🤖 Paper mode`, `⚡ Execution mode`, `⚠️ Risk status`, `🚨 Emergency stop`.

## Portfolio sync

При включенном `BCS_API_ENABLED=true` бот каждые 60 секунд запускает read-only синхронизацию портфеля и сделок:

- `portfolioSync` сохраняет snapshot портфеля и позиции в SQLite.
- `tradeSync` забирает сделки и пишет их с deduplication по external id.
...
- В коде нет автоматического выставления заявок, market orders запрещены, а BCS API layer запрещает executeOrder().
